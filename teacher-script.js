...
async function checkExistingSession() {
    ...
    if (session) {
        ...
        if (teacher) {
            ...
            // ✅ Force load quests and contests
            await renderAllQuestAccordions();
            await loadTeacherContests();
            
            // ✅ Also load the quests accordion container
...
async function loadContestProfile(contest) {
    ...
    // Date formatting - use contest.start_date directly
    ...
    if (!isCreator) {
        ...
        if (hideBtn) hideBtn.addEventListener('click', () => hideContestFromMyStudents(contest.id));
        if (unhideBtn) unhideBtn.addEventListener('click', () => unhideContestForMyStudents(contest.id));
    } else {
        const deleteBtn = document.getElementById('delete-contest-btn');
        if (deleteBtn) deleteBtn.addEventListener('click', () => deleteContest(contest.id));
    }
}
...
async function confirmRejection() {
    ...
    // Get the submission to find student ID
...
async function deleteContest(contestId) {
    ...
    // Delete the contest (cascade will delete submissions and votes)
    const { error } = await window.supabase
        .from('art_battle_contests')
        .delete()
        .eq('id', contestId);
...
function openDateModal(date, existingEntry) {
    ...
    const notesInput = document.getElementById('modal-notes');  // NEW
    ...
    if (existingEntry) {
        ...
        notesInput.value = existingEntry.notes || '';  // NEW
